...
public class LocationValidator {
    ...
    private static double getGreatCircleDistance(double latitude1, double longitude1, double latitude2, double longitude2) {
        ...
        double res = 6371 * Math.acos(Math.sin(x1) * Math.sin(x2) + Math.cos(x1) * Math.cos(x2) * Math.cos(y1 - y2));
        Log.e("Distance", "getGreatCircleDistance: " + res);
        return res;
    }

    public static boolean checkIfNearby(Article article, Location location) {
        Log.e("Article", "checkIfNearby: " + article.getStoreLatitude() + " - " + article.getStoreLongitude());
        return (getGreatCircleDistance(article.getStoreLatitude(), article.getStoreLongitude(), location.getLatitude(), location.getLongitude()) < NEARBY_RADIUS);
    }
